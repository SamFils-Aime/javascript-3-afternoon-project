...
//Code Here
let totalPopulation  = populations.reduce(less)
function less (tot,val){
  return tot+val
}
...
//Code Here
let myStrongest  = monstersInYourPocket.filter(hey)
function hey (el){
  if (el.CP > 200){return el.monster} 
}
...
let orderTotals = orders.map(tax)
function tax (el) { return el.price+el.price*el.tax}
...
let bobsTotal=  totals(purchases)
